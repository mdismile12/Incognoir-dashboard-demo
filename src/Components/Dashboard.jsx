...
const Dashboard = ({ demoMode }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/api/summary")
      ...
      .then((data) => {
        ...
        setDashboardData(data.data); // kyunki backend me { success, data }
      })
  ...
  useEffect(() => {
    ...
    const updateData = () => {
      ...
      console.log(
        `🔄 Update #${updateCounter + 1} at ${now.toLocaleTimeString()}`,
      );
  ...
  const alerts = [
    ...
    },
    {
      id: 2,
      type: "warning",
      title: "Device health score below threshold",
      time: "15 minutes ago",
      location: "Device ID: SEC-482",
    },
    {
      id: 3,
      type: "info",
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          <div>
            ...
            <div className="mt-2 text-sm text-yellow-400">
              Updates: {updateCounter} | Last:{" "}
              {lastUpdateTime.toLocaleTimeString()}
            </div>
          </div>
          <div className="mt-4 md:mt-0 flex items-center space-x-4">
            <div className="text-sm text-gray-400">Auto-refresh: 2s</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <StatCard
          ...
          value={Number(dashboardData?.totalSessions ?? 0)}
          change={-3.2}
          trend="down"
          icon="shield"
          color="blue"
        ...
        <StatCard
          ...
          value={Number(dashboardData?.avgSessionDuration ?? 0)}
          change={8.2}
          trend="up"
          icon="clock"
          color="blue"
        ...
        <StatCard
          ...
          value={Number(dashboardData?.botPercentage ?? 0)}
        ...
        <StatCard
          title="Avg Requests per Session"
          value={Number(dashboardData?.avgRequests ?? 0)}
        ...
        <StatCard
          ...
          // value="142"
          value={Number(dashboardData?.avgErrorRate ?? 0)}
          unit="ms"
          change={-8.1}
          trend="down"
          icon="speed"
        ...
        />
        <StatCard
          title="Avg Repetition Rate"
          // value="142"
          value={Number(dashboardData?.avgRepetitionRate ?? 0)}
          unit="ms"
          change={-8.1}
          trend="down"
          icon="speed"
        ...
        />
        <StatCard
          title="Avg Unique Paths"
          // value="142"
          value={Number(dashboardData?.avgUniquePaths ?? 0)}
          unit="ms"
          change={-8.1}
          trend="down"
          icon="speed"
        ...
        <StatCard
          title="Avg Time Between Requests"
          value={Number(dashboardData?.avgTimeBetweenRequests ?? 0)} 
          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              <tr className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                ...
                <td className="py-3 px-4 text-white">
                  Device health check initiated
                </td>
                <td className="py-3 px-4 text-gray-400">Monitoring System</td>
                <td className="py-3 px-4">
                  <span className="px-2 py-1 text-xs bg-yellow-500/20 text-yellow-300 rounded-md">
                    In Progress
                  </span>
                </td>
              </tr>
